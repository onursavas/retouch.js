...
| Tool | Description |
|------|-------------|
| **Crop** | Commit-style: drag a selection (free-form or fixed aspect — 16:9, 4:3, 1:1, 3:2, 9:16), then **Apply crop** (or press Enter) — the discarded area disappears everywhere; **Reset crop** brings the full frame back. Rule-of-thirds grid, rotate-90° and flip H/V that keep the crop over your content. Straighten auto-crops to the largest inscribed window (no black corners), and vertical/horizontal **perspective (keystone) correction** fixes converging lines. Works on video too. |
