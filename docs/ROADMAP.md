...
| ⬜ | P1 | Keyboard: Esc=cancel, Enter=done, tool hotkeys, arrow-nudge crop/trim handles |
| ⬜ | P1 | Focus trap + `role="dialog"` + ARIA labels in editor; full keyboard operability |
| ⬜ | P1 | Error feedback: toast on rejected files (type/size/count) + `file:rejected` event |
| ⬜ | P1 | Before/after compare (hold button) · Reset-all / per-tool reset |
| ⬜ | P2 | Zoom & pan canvas (wheel/pinch, fit/100%) |
| ⬜ | P2 | EXIF orientation auto-correct on load |
...
| ⬜ | P2 | Export options: format png/jpeg/webp, quality, max dimensions; `exportOne(id)` |
